...
class ModelsBulkTask {
  /// Returns a new [ModelsBulkTask] instance.
  ModelsBulkTask({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ModelsBulkTask &&
  ...
  @override
  String toString() => 'ModelsBulkTask[assignees=$assignees, attachments=$attachments, bucketId=$bucketId, coverImageAttachmentId=$coverImageAttachmentId, created=$created, createdBy=$createdBy, description=$description, done=$done, doneAt=$doneAt, dueDate=$dueDate, endDate=$endDate, hexColor=$hexColor, id=$id, identifier=$identifier, index=$index, isFavorite=$isFavorite, labels=$labels, percentDone=$percentDone, position=$position, priority=$priority, projectId=$projectId, reactions=$reactions, relatedTasks=$relatedTasks, reminders=$reminders, repeatAfter=$repeatAfter, repeatMode=$repeatMode, startDate=$startDate, subscription=$subscription, taskIds=$taskIds, title=$title, updated=$updated]';
  ...
  /// Returns a new [ModelsBulkTask] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ModelsBulkTask? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ModelsBulkTask[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ModelsBulkTask[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ModelsBulkTask(
        ...
        taskIds: json[r'task_ids'] is Iterable
            ? (json[r'task_ids'] as Iterable).cast<int>().toList(growable: false)
            : const [],
  ...
  static List<ModelsBulkTask> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ModelsBulkTask>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ModelsBulkTask.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ModelsBulkTask> mapFromJson(dynamic json) {
    final map = <String, ModelsBulkTask>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ModelsBulkTask.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ModelsBulkTask-objects as value to a dart map
  static Map<String, List<ModelsBulkTask>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ModelsBulkTask>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ModelsBulkTask.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
